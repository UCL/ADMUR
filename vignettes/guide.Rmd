...
c2 <- 'firebrick'
lwd <- 3
pdf.breaks <- seq(0,0.0015, length.out=30)
yr.breaks <- seq(5500,7500,length.out=30)
hist(hinges$yr2,border=c1,breaks=yr.breaks);abline(v=CPL$year[2],col=c2,lwd=lwd)
hist(hinges$yr3, border=c1,breaks=yr.breaks);abline(v=CPL$year[3],col=c2,lwd=lwd)
hist(hinges$pdf1, border=c1,breaks=pdf.breaks);abline(v=CPL$pdf[1],col=c2,lwd=lwd)
hist(hinges$pdf2, border=c1,breaks=pdf.breaks);abline(v=CPL$pdf[2],col=c2,lwd=lwd)
hist(hinges$pdf3, border=c1,breaks=pdf.breaks);abline(v=CPL$pdf[3],col=c2,lwd=lwd)
hist(hinges$pdf4, border=c1,breaks=pdf.breaks);abline(v=CPL$pdf[4],col=c2,lwd=lwd)
```
