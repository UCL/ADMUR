...
```{r, eval = TRUE, fig.height = 4, fig.width=7, fig.align = "center", dev='svg', warning=FALSE}
library(DEoptimR)
best <- JDEoptim(lower=rep(0,5), upper=rep(1,5), fn=objectiveFunction, PDarray=PD, type='CPL',maxiter=30)
years <- as.numeric(row.names(PD))
pdf <- convertPars(best$par,years)
SPD <- summedPhaseCalibrator(data, shcal13, c(5500,7500))
plotPD(SPD)
lines(5500:7500, toy$model$pdf, lwd=2, col='steelblue')
lines(pdf$year,pdf$pdf,lwd=2,col='firebrick')
legend(x=6300,y=max(pdf$pdf),cex=0.7,lwd=2,col=c('steelblue', 'firebrick'),bty='n',legend=c('toy','best fitted 3-CPL'))
```

Notice the maximum log likelihood for this best fit 3-CPL model is better (higher) than for the toy model from which the dataset was randomly sampled:

```{r, eval = TRUE}
-best$value
loglik(PD, toy$model)
```
This is because the dataset was a small random sample (N=99) generated from the toy model. Therefore the best fitting model should indeed be slightly different to the toy model.
