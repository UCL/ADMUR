...
1. **Sinusoidal** 3->  parameters: f, p, r. 
1. **Logistic** -> 2 parameters: k, x$_0$. 
...
```{r, eval = TRUE, fig.height = 7, fig.width=7, fig.align = "center", dev='svg', warning=FALSE}
# convert parameters to model PDs
...
lines(mod.cauchy, col=cols[4], lwd=2)
legend(x=7500, y=max(spd1), cex=0.7, lwd=2, col=cols[c(1,4)], bty='n', legend=c('Gaussian','Cauchy'))

plotPD(spd2)
lines(mod.sine, col=cols[1], lwd=2)
lines(mod.uniform, col=cols[4], lwd=2)
legend(x=7500, y=max(spd2), cex=0.7, lwd=2, col=cols[c(1,4)], bty='n', legend=c('Sinewave','Uniform'))

plotPD(spd3)
lines(mod.logistic, col=cols[1], lwd=2)
lines(mod.exp, col=cols[4], lwd=2)
legend(x=7500, y=max(spd3), cex=0.7, lwd=2, col=cols[c(1,4)], bty='n', legend=c('Logistic','Exponential'))
```
