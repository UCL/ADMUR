...
Note: The Area Breaking Algorithm is a heuristic that performs well and ensures the maximum likelihood parameters are always found, however its mapping of random parameters to PDF coordinates is not perfectly even, and we welcome ideas for a more elegant algorithm.
...
The function [mcmc()](../html/mcmc.html) uses the Metropolis-Hastings algorithm to search joint parameter values of an n-CPL model, given a the calibrated probability distributions of phases in a ^14^C dataset.
...
The raw output of one mcmc chain are the 2n-1 parameters between 0 and 1, and distributions can be plotted. Note, because the dataset is small (N = 99) the credible regions are wide.

```{r, eval = TRUE, fig.height = 4, fig.width=7, fig.align = "center", dev='svg', warning=FALSE}
require(scales)
par(mfrow = c(1,2), mar = c(4,4,2,2))
plot(chain$res[,1], chain$res[,3], pch = 16, col = alpha(1,0.15))
points(best$par[1], best$par[3], col = 'red', pch = 16, cex = 1.5)
plot(chain$res[,1], chain$res[,4], pch = 16, col = alpha(1,0.15))
...
```{r, eval = TRUE}
hinges <- convertPars(chain$res, 5500:7500)
```

Again distributions can be viewed, however it is important to note that these are only marginal distributions (attempts to view a 5-dimensional distribution in just 2 dimensions) so their complex shape cannot adequately show the true 5D densities. Therefore the true maximum likelihood (red points) may not perfectly match the densest part of the cloud. 

```{r, eval = TRUE, fig.height = 4, fig.width=7, fig.align = "center", dev='svg', warning=FALSE}
par(mfrow=c(1,2) , mar = c(4,4,2,2))
plot(hinges$yr2, hinges$pdf2, pch=16, col=alpha(1,0.15), ylim=c(0,0.0006))
points(CPL$year[2], CPL$pdf[2], col='red', pch=16, cex=1.5)
plot(hinges$yr3, hinges$pdf3, pch=16, col=alpha(1,0.15), ylim=c(0,0.0014))
...
Continue using this toy example, re-run MLE for various other models, and plot BIC
