...
For each dataset (simulated and observed) we generate an SPD and use a statistic that measures how divergent each SPD is from expectation, by calculating the total probability that sits outside the 95% CI, scaled by the local standard deviation. 

```{r, eval = FALSE}
summary <- SPDsimulationTest(data, 
	calcurve=shcal20, 
	calrange=c(5500,7500), 
	pars=best$par, 
	type='CPL', 
	N=20000)
```
...
```{r, echo = FALSE}
load('C:/Users/adrian/Documents/FILES/analysis/spanning/ramiro/2019/github/vignette.3CPL.SPDsimulationTest.RData')
```
...
The generational rate $r$ of this $i^{th}$ piece is $$r_i=1-exp[\ln(\frac{y_1}{y_2})/\frac{x_1-x_2}{25}]$$ and the expected rate across the entire line as $N$ approaches +$\infty$ is $$\sum_{i=1}^{N}r_i/N$$

For example, a population decline from n=200 to n=160 across 100 years is conventionally considered to have a generational (25 years) decline rate of $1-exp[\ln(\frac{160}{200})/\frac{100}{25}]$ = 5.426% loss per generation.
If partitioned into just $N=2$ equal sections (n=200, n=180, n=160), we require two generational decline rates:
$1-exp[\ln(\frac{180}{200})/\frac{50}{25}]$ = 5.132% and  $1-exp[\ln(\frac{160}{180})/\frac{50}{25}]$ = 5.719%, giving a mean of 5.425%. 
As the number of sections $N$ approaches +$\infty$, the mean rate asymptoticly approaches 5.507%. 
...
An exponential curve between n=200 and n=1 across the same 100 years has a decline rate of $1-exp[\ln(\frac{1}{200})/\frac{100}{25}]$ = 73.409% loss per 25 yr generation.
...
Part 2 provided a framework to directly select the best model from  dataset, from CPL models, exponential, uniform, or in theory any other model. This contrasts with the SPD simulation methodology which requires the researcher to *a priori* specify a single null model, then generate many simulated datasets under this null model which are compared with the observed dataset to generate a p-value. This SPD simulation approach has several shortcomings which render it inferior to the CPL modelling approach: 

1. Recent studies increasingly suggest population fluctuations are ubiquitous thought history, rendering the application of a null model inappropriate. In contrast this new methodology does not assume a null, and instead allows any number of models to be compared. 
1. A low p-value merely allows us to reject (or fail to reject) the tested model, but does not provide us with a plausible alternative explanation. This leaves an inferential vacuum in which it is common for researchers to assign colourful demographic narratives to periods outside the 95% CI, but these are only interpretive and not directly supported by the test statistic. Instead the CPL framework provides a single best explanation.
...
Therefore the summary generated in the previous section *'Goodness of fit test'* provides a number of other useful outputs that can be plotted, including:

1. the SPD vector
1. the model vector
1. expected simulation vector (mean of simulations)
1. confidence intervals (various quantiles of simulations)
1. p-value for Goodness of Fit test
1. summary statistic of observed dataset
1. summary statistic of each simulated dataset
1. Index of SPD values that are above or below the 95% CI (+1 and -1 respetively, or 0 if within the 95% CI)
1. effective number of 14C dates within the specified date range (not the total sample size of the dataset)
1. effective number of phases in the specified date range

The function ???? represents these data in a single plot:
