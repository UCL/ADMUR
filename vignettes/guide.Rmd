...
1. **Continuous Piecewise Linear (CPL) Modelling:** Maximum Likelihood Parameter estimation using the likelihood function. MCMC for estimating credible intervals. Model selection using BIC, and goodness-of-fit (GOF) test.
...
![Illustration of the toy 3-CPL model PD, described using just four coordinate pairs (hinges).](model plot.svg)
...
```{r, echo = FALSE}
load('C:/Users/adrian/Documents/FILES/analysis/spanning/ramiro/2019/github/vignette.3CPL.JDEoptim.best.RData')
```
...
```{r, eval = FALSE}
chain <- mcmc(PDarray=PD, startPars=best$par, type='CPL', N=30000, burn=2000, thin=5, jumps =0.05)
```

```{r, echo = FALSE}
load('C:/Users/adrian/Documents/FILES/analysis/spanning/ramiro/2019/github/vignette.3CPL.MCMC.RData')
```
...
c2 <- 'firebrick'
lwd <- 3
pdf.brk <- seq(0,0.0015, length.out=30)
yr.brk <- seq(5500,7500,length.out=30)
...
plot(hinges$yr2, hinges$pdf2, pch=16, col=alpha(1,0.03), ylim=c(0,0.0005))
points(CPL$year[2], CPL$pdf[2], col='red', pch=16, cex=1.2)
plot(hinges$yr3, hinges$pdf3, pch=16, col=alpha(1,0.03), ylim=c(0,0.0015))
...
The objective function returns the negative log-likelihood since the search algorithm seeks to minimise the objective function. It is therefore trivial to extract the log-likelihoods, and calculate the BIC scores using the formula $BIC=k\ln(n)-2L$ where $k$ is the number of parameters, $n$ is the effective sample size (i.e. the number of phases = 211), and $L$ is the maximum log-likelihood.
...
it is crucial to test if the selected model is plausible, or in other words, to test if the observed data is a reasonable outcome of the model. If the observed data is highly unlikely, then the model must be rejected, even if it was the best model selected.
...
For complex data types the choice of a suitable summary statistic can often be difficult, as different statistics represent different aspects of the data, and therefore can vary in plausibility under a selected model. We avoid this problem by using the likelihood (the probability of our data given the selected model) as our summary statistic, which provides a direct quantification of how 'extreme' the data are. As such, the proportion of simulated datasets with a lower probability than the observed dataset provides the perfect definition of *more extreme*. 

The GOF test below provides a p-value of 0.44 for the best model (3-CPL) providing a sanity check that the data cannot be rejected under this model, and therefore is a plausible model:

```{r, eval = FALSE}
p <- GOF(data, calcurve=shcal20, calrange=c(5500,7500), pars=best$par, type='CPL')
```

```{r, echo = FALSE}
load('C:/Users/adrian/Documents/FILES/analysis/spanning/ramiro/2019/github/vignette.3CPL.GOF.RData')
```
...
1. CPL modelling allows for the possibility of hiatus periods, defined by pieces between hinges with a zero or near zero PD. Conventionally, the total percentage change from ay value to zero is a 100% decrease, however the equivalent percentage increase from zero is undefined.
...
The generational rate $r$ of this $i^{th}$ piece is $$r_i=e^{\ln(\frac{y_1}{y_2})/\frac{x_1-x_2}{25}}$$ and the expected rate across the entire line is $$\sum_{i=1}^{N}r_i/N-1$$

For example, a population decline from n=200 to n=160 across 100 years is conventionally considered to have a generational (25 years) decline rate of $1-exp(\ln(160/200)/(100/25))$ = 5.42584% loss per generation.
If partitioned into just two equal sections (n=200, n=180, n=160), we require two generational decline rates:
$1-exp(log(180/200)/(50/25))$ = 5.13167% and $1-exp(log(160/180)/(50/25))$ = 5.71910%, giving a mean of 5.42538%. 
As the number of sections $N$ approaches infinity, the mean rate asymptoticly approaches 5.50724%. 
The similarity to the conventional rate of 5.42584% is because the total percentage loss is small (20%), therefore an exponential curve between n=200 and n=160 is similar to a straight line.
...
An exponential curve between n=200 and n=1 across the same 100 years has a decline rate of $1-exp(log(1/200)/(100/25))$ = 73.409% loss per 25 yr generation.
Meanwhile a linear model between n=200 and n=1 across the same 100 years has an expected decline rate of 47.83539% loss per generation.
...
Part 2 provides a framework to directly estimate the best model from  dataset, including CPL models, exponential, uniform, or in theory any other model. In contrast, a less sophisticated albeit currently common approach the SPD simulation methodology. This requires the researcher to *a priori* specify a single null model, then generate many simulated datasets under this null model which are compared with the observed dataset to generate a p-value. This SPD simulation approach has several shortcomings which render it inferior to the CPL modelling approach: 

1. Recent studies increasingly suggest population fluctuations are ubiquitous thought history, rendering the concept of a single null model untenable for this problem. In contrast this new methodology does not assume a null, and instead allows any number of models to be compared. 
1. A low p-value merely allows us to reject the null, but does not provide us with a plausible alternative explanation. It is common for researchers to assign colourful demographic narratives to periods outside the 95% CI, but these are only interpretive and not directly supported by the test statistic. Instead the CPL framework provides a single best explanation, in the form of specific demographic events (hinge coordinates), event dates, growth rates, and CIs.
1. Fitting the null model (and therefore estimating its parameters) is commonly achieved by discretising the SPD, then incorrectly assuming these points somehow represent data points, to which the null model is fitted my minimising their residuals. In contrast, the CPL framework correctly calculates the probability of the data given some proposed model parameters. 
1. Generating a meaningful p-value relies on the use of one or more suitable summary statistics that can appropriately summarise an entire dataset (or SPD), such that the p-value fairly describes the probability of the null model generating a dataset that is *as or more extreme* than the observed dataset. Commonly used summary statistics are a variation on the theme of the proportion of the SPD that falls outside the simulated confidence interval. In contrast, the CPL framework uses a Goodness Of Fit (GOF) test that uses the likelihood, providing a direct and complete measure of *as or more extreme*.

Despite these short comings, and purely for comparative purposes, the ADMUR package also provides tools to apply the SPD simulation methodology, with a key improvement that resolves problems 3 and 4 above. Specifically, we calculate the null model likelihood given the data. Therefore the null model parameters are fitted to the data using the maximum likelihood; and the p-value is calculated using the distribution of null model likelihoods given each simulated dataset. 
