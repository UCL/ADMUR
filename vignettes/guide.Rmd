...
A [toy()](../html/toy.html) model and population is provided to demonstrate how this achieved. Notice the toy population covers a slightly wider date range to ensure simulated 14C dates are well represented around the edges.

(need to first demonstrate the likelihood with a single date)

## Likelihood of a dataset

First, we simulate a plausible 14C dataset and calibrate it:

```{r, eval = TRUE}
# generate some simulated archaeological samples in proportion to the toy population dynamics

cal <- sample(x = toy$population$calBP, size = 99, replace = TRUE, prob = toy$population$individuals ) 

# convert to plausible 14C dates, using a real dataset to simulate plausible 14C errors
...
# calibrate each phase (one date per phase), taking care to restrict to the modelled date range
CalArray <- makeCalArray(shcal13, calrange = range(toy$model$calBP))
x <- phaseCalibrator(data, CalArray)
```
