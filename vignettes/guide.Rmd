...
```{r, eval = TRUE}
years <- as.numeric(row.names(toy$model))
uniform.model <- data.frame(pdf=dunif(years,min(years),max(years)))
row.names(uniform.model) <- years
loglik(PD=PD, model=uniform.model)
```

And indeed the toy model is forty million trillion times more likely than the uniform model:
```{r, eval = TRUE}
...
```{r, eval = FALSE}
chain <- mcmc(PDarray=PD, startPars=best$par, N=30000, burn=2000, thin=5, jumps =0.05)
```
...
lwd <- 3
breaks <- 50
hist(hinges$yr2, border=c1);abline(v=CPL$year[2],col=c2,lwd=lwd,breaks=breaks)
hist(hinges$yr3, border=c1);abline(v=CPL$year[3],col=c2,lwd=lwd,breaks=breaks)
hist(hinges$pdf1, border=c1);abline(v=CPL$pdf[1],col=c2,lwd=lwd,breaks=breaks)
hist(hinges$pdf2, border=c1);abline(v=CPL$pdf[2],col=c2,lwd=lwd,breaks=breaks)
hist(hinges$pdf3, border=c1);abline(v=CPL$pdf[3],col=c2,lwd=lwd,breaks=breaks)
hist(hinges$pdf4, border=c1);abline(v=CPL$pdf[4],col=c2,lwd=lwd,breaks=breaks)
```
...
points(CPL$year[2], CPL$pdf[2], col=c2, pch=16, cex=1.2)
plot(hinges$yr3, hinges$pdf3, pch=16, col=alpha(1,0.03), ylim=c(0,0.0015))
points(CPL$year[3], CPL$pdf[3], col=c2, pch=16, cex=1.2) 
```
...
data.frame(L1= -CPL.1$value,
	...
	Lexp= -exp$value,
	Lunif= -uniform)
BIC.1 <- 1*log(294) - 2*(-CPL.1$value)
BIC.2 <- 3*log(294) - 2*(-CPL.2$value)
BIC.3 <- 5*log(294) - 2*(-CPL.3$value)
BIC.4 <- 7*log(294) - 2*(-CPL.4$value)
BIC.exp <- 1*log(294) - 2*(-exp$value)
...
CPL4 <- convertPars(pars=CPL.4$par, years=mod.years, type='CPL')  
